    ...
    for i_batch, (x, y) in enumerate(loader):
        ...
        for metric_name, metric_fn in batch_metrics.items():
            if metric_name == 'fscore':
                # fscore is binary but our output has 2 classes
                # so we need to convert it to binary by just getting the first class
                #print(y_pred.shape, y.shape)
                y_pred_softmax = torch.nn.functional.softmax(y_pred, dim=1)
                y_pred_output = torch.argmax(y_pred_softmax, dim=1)
                # cast to int
                y_pred_binary = (1 - y_pred_output).round()
                ...
                # 1 0
                # 0 1
                # binary is reverse of y
                y_binary = 1 - y # 0 for haruna 1 for not haruna
                metrics_batch[metric_name] = metric_fn(y_pred_binary, y_binary).detach().cpu()
                # metrics_batch[metric_name] = metrics_batch[metric_name][0]
            else:
